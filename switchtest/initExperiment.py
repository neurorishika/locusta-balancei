...
call(['python', homepath+'locusta-balancei/switchtest/receptorLayer.py', odor_path, protocol_path, locust_path, folder])
...
for i in range(data['n_split']):
    call(['python',homepath+'locusta-balancei/switchtest/antennalLobe.py',str(i), locust_path, protocol_path, sys.argv[5], folder, sys.argv[6], sys.argv[7]])
...
copy(homepath+"locusta-balancei/switchtest/antennalLobe.py",folder)
